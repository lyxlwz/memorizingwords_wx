<template>
  <view class="register_page">
    ...
    <view
      class="input_box triangle"
      :class="[type == 1000 ? 'left_triangle': 'right_triangle']"
    >
      <input
        type="text"
        v-model="email"
    ...
    <view class="password_register">
      ...
      <text
        v-if="type == 1000"
        @click="onPageJump('/pages/user/forget')"
      >忘记密码？</text>
    </view>
    <!-- #ifdef APP-PLUS -->
    <view
      class="third_party_login_box"
      v-if="isWeixin || (isIos && system >= 13)"
    >
      <view class="third_party_title"><text>第三方登录</text></view>
      <view class="third_party_content">
        <image
          src="../../static/icon/ic_login_weixin.png"
          v-if="isWeixin"
          @click="onWxAppLogin"
          mode="aspectFit"
        ></image>
        <image
          src="../../static/icon/ic_login_ios.png"
